    ...
    <div className="container max-w-4xl py-10">
      ...
      <Card className="mb-8">
        <CardHeader>
          ...
          <CardDescription>Configure the database for user registration and OTP verification</CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            {/* Status message box */}
            {setupStatus.message && (
              <div className={`rounded-md ${setupStatus.success ? "bg-green-50 text-green-600" : "bg-destructive/15 text-destructive"} p-4 mb-4`}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-lg">
                    {setupStatus.success ? <CheckCircle2 className="h-5 w-5" /> : <AlertCircle className="h-5 w-5" />}
                  </span>
                  <span className="font-semibold">{setupStatus.success ? "Success" : "Error"}</span>
                </div>
                <p>{setupStatus.message}</p>
                {setupStatus.error && (
                  <pre className="mt-2 bg-muted p-2 rounded text-xs overflow-auto">{setupStatus.error}</pre>
                )}
              </div>
            )}
            ...
            <p className="text-sm text-muted-foreground mb-4">
              For quicker setup, you can use the direct setup method which bypasses the dependency on PostgreSQL functions.
            </p>
            ...
            <div className="ml-2 mb-4 flex flex-col gap-2">
              ...
              >
                ...
                {setupStatus.message === "Setting up database..." ? "Setting up..." : "Run Direct Setup"}
              </Button>
