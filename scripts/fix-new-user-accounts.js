...
async function fixNewUserAccounts() {
  ...
  try {
    ...
    if (unconfirmedUsers.length === 0 && pendingUsers.length === 0) {
      console.log('No users need fixing. All users have confirmed emails and no users are pending.');
      process.exit(0);
    }
    
    console.log('\nUsers without email confirmation:');
    unconfirmedUsers.forEach(user => {
      console.log(`- ${user.email} (ID: ${user.id}, Status: ${user.user_metadata?.status || 'none'}, Role: ${user.user_metadata?.role || 'none'})`);
    });
    
    console.log('\nUsers with pending status:');
    pendingUsers.forEach(user => {
      console.log(`- ${user.email} (ID: ${user.id}, Email confirmed: ${user.email_confirmed_at ? 'Yes' : 'No'}, Role: ${user.user_metadata?.role || 'none'})`);
    });
