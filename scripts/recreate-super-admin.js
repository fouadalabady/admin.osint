...
async function recreateSuperAdmin() {
  ...
  try {
    ...
    if (sqlError) {
      ...
      console.log('The user has still been created, but you may encounter email confirmation issues.');
    } else {
      console.log('Email confirmation time set via SQL.');
    }
    
    // Verify the user creation
    const { data: verifyData, error: verifyError } = await supabase.auth.admin.getUserById(newUser.user.id);
    ...
    console.log('IMPORTANT: The password has been set to "StrongPassword123!". Please change it after login.');
