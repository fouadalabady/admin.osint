...
async function fixAllUserEmails() {
  ...
  try {
    ...
    const unconfirmedUsers = userList.users.filter(
      user => user.email_confirmed_at === null || new Date(user.email_confirmed_at).getTime() === 0
    );
    ...
    for (const user of unconfirmedUsers) {
      ...
      if (hasVerifiedOtp || hasVerifiedRegistration) {
        console.log(
          `User ${user.email} has verified their email but email_confirm is false. Fixing...`
        );
    ...
    if (unconfirmedUsers.length - fixedCount > 0) {
      console.log(
        `\nNote: The remaining ${
          unconfirmedUsers.length - fixedCount
        } users have not verified their email through OTP yet.`
      );
      console.log(
        `They will need to complete the email verification process or have their email manually confirmed.`
      );
    }
