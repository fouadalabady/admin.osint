...
async function confirmUserEmail() {
  ...
  try {
    ...
    const { data: updatedData, error: verifyError } = await supabase.auth.admin.getUserById(
      user.id
    );
    ...
    if (updatedData.user.email_confirmed_at) {
      ...
      console.log(
        'Warning: Email confirmation timestamp not set. The user may still have issues logging in.'
      );
